...
public class FamilyTest {
	...
	public static void main(String[] args) {
		...
		//runHql("from Parent p where p.name='홍길동'");
		//runHql2("from Parent p where p.name=?", "홍길동");
		//runHql2("from Parent p where p.name=:name", "홍길동");

		//runHql("select c.name, c.p.name from Child c where c.name='이미자'");
